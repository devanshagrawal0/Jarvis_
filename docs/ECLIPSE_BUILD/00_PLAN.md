...
- [ ] **P2·W6 — Full Agent Foundry.** Generated ephemeral Worker personas from the blueprint
  schema, qualification pipeline, agent reputation + promotion, Memory Curator agent.
- [ ] **P2·W7 — Branch intelligence (Lattice Explorer).** Policy router (beam/tree/counterfactual/
  debate/GoT), branch-value economics + pruning, verifier-guided targeted repair. **Gate:**
  Totality quality gain > added-cost threshold vs the Part-1 baseline.
- [ ] **P2·W8 — Full Artifact Reactor.** Canonical content graph + all formats (DOCX/PPTX/PDF/
  XLSX/code/site) + render/QA verification + audience transformers + dependency regeneration + Nano-Banana figures.
- [ ] **P2·W9 — Full product surfaces.** Lattice Explorer, Agent Constellation, Evidence War
  Room, Memory Lens, Budget Reactor, Artifact Foundry, Debrief — the full spatial mission workspace over real events.
- [ ] **P2·W10 — Advanced memory, promotion & evolution.** HippoRAG entity-graph layer, RAPTOR
