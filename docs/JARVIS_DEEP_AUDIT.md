...
| **A-13** | The index emitted a deterministic HMAC of every character trigram of every word into a plain table beside the ciphertext. The danger was not the hashing but the **sliding window**: consecutive trigrams of a word overlap by two characters, so recovered trigrams chain into words and words into sentences — over a space of only ~17k values with famously stable English frequencies. An attacker with the DB file but not the DPAPI-wrapped master key could reconstruct much of the indexed text without touching AES-GCM. Replaced with **one stem token per word** (first 5 chars, words longer than 5): no overlap structure to chain, a 26⁵ space rather than ~17k, and tokens per word drop from (length−2) to 2 — collapsing the co-occurrence graph the attack feeds on. | **Executed on the live store, not just fixed in code** (the leak lives in the stored rows, not in the emitter): **1,028,590 trigram tokens removed across all 2,478 rows, 0 remaining**, after a 148 MB backup. Backend restarts onto the same shadow session, 0 errors. Mutation: restoring the trigram window turns 3 tests red. **A capability regression caught and corrected mid-fix:** removing trigrams outright broke recall — `projects` stopped matching `project`, failing an existing test. The stem token exists because of that, and a test now pins inflection matching so a future "simplification" back to word-only tokens fails loudly. **Honest limitation:** purged rows carry word tokens only, since the purge has no plaintext to derive stems from; they regain stem matching when re-indexed. |
