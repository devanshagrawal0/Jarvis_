...
function createCapabilityEngine({
  ...
  const definitions = [
    ...
    ["ui_resize_widget", "Resize a JARVIS widget: size 'small', 'medium', or 'large'; or 'expand'/'maximize' to full focus; or 'minimize'. To bring a focused/expanded widget BACK to normal (exit focus mode — the owner says 'normal view', 'standard view', 'unfocus', 'un-expand', 'restore', 'make it normal'), call this with size 'medium'. If the owner named a widget, pass its id; otherwise OMIT id to target the focused widget. Never ask which widget when one is on screen.", "observe", false],
