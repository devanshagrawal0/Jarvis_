...
function createPersonalContextRouter({ store, trustedOwnerCloud = false } = {}) {
  ...
  function route(input = {}) {
    ...
    const facts = hits.filter((hit) => hit.content?.predicate).map((hit) => ({ predicate: hit.content.predicate, value: hit.content.value, unit: hit.content.unit || null, freshness: hit.freshness, source: { recordType: hit.recordType, recordId: hit.recordId, recordVersion: hit.recordVersion } }));
    ...
    return { query, topics: expanded.topics, plan, retrieval: { first: first.channels, graph: { skipped: graphResult.skipped, mode: graphResult.mode, hits: graphResult.hits.length }, second: second.channels, selected: hits.length }, pack, facts, contextText, privacy: { trustedOwnerCloud: cloudAllowed, eligibleSensitivities: [...allowed] }, providerCalls: 0, costUsd: 0, ...(input.includeDiagnostics ? { diagnostics: timings } : {}) };
  }
