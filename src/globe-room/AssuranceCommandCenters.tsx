...
export function ConnectionsCommandCenter({ data, loading }: { data: any; loading: boolean }) {
  ...
  // Wave 4 — progressive Google connect: open consent for ONLY the requested capability bundle.
  const connectGoogle = async (bundle: string) => {
    try {
      ...
      else jarvis(`I tried to start the Google ${bundle} connection but got no authorization URL. What's blocking it?`);
    } catch { jarvis("The Google connection couldn't start — check that OAuth credentials are configured."); }
  };
  ...
  return <div className="as-center">
    ...
    <div className="as-split"><section className="as-panel"><header><div><span>Provider matrix</span><strong>{filtered.length} services</strong></div><small>latency</small></header><div className="as-provider-list">{loading && !providers.length ? <div className="as-empty">Synchronizing providers…</div> : filtered.map(([key, provider]) => <button key={key} data-selected={selectedEntry?.[0] === key} onClick={() => setSelectedKey(key)}><i data-live={provider.connected} /><div><strong>{provider.label || key}</strong><span>{provider.model || provider.authMode || provider.source || "unconfigured"}</span></div><small>{Number.isFinite(provider.latencyMs) ? `${provider.latencyMs}ms` : "—"}</small></button>)}</div></section>
      <section className="as-panel as-detail"><header><div><span>Connection inspector</span><strong>{selected?.label || "No provider"}</strong></div>{selected ? <small>{selected.connected ? "connected" : selected.validationState || "missing"}</small> : null}</header>{selected ? <div className="as-body"><div className={`as-state ${selected.connected ? "live" : "blocked"}`}><i /><div><span>{selected.connected ? "Operational" : "Not operational"}</span><strong>{selected.connected ? selected.source || "connected" : selected.lastError || `Missing ${(selected.missing || []).join(", ") || "configuration"}`}</strong></div></div><div className="as-grid"><div><span>Model</span><strong>{selected.model || "none"}</strong></div><div><span>Auth</span><strong>{selected.authMode || "not reported"}</strong></div><div><span>Environment</span><strong>{selected.environment || selected.source || "unknown"}</strong></div><div><span>Latency</span><strong>{Number.isFinite(selected.latencyMs) ? `${selected.latencyMs}ms` : "not measured"}</strong></div><div><span>Last request</span><strong>{when(selected.lastRequestAt)}</strong></div><div><span>Last tool</span><strong>{selected.lastToolCall || "none"}</strong></div></div>{selected.baseUrl ? <div className="as-copy"><span>Endpoint</span><code>{selected.baseUrl}</code></div> : null}{selected.scopes?.length ? <div className="as-section"><span>Authorized scopes</span><div className="as-tags">{selected.scopes.map((scope: string) => <code key={scope}>{scope.replace("https://www.googleapis.com/auth/", "")}</code>)}</div></div> : null}{selected.catalog ? <div className="as-section"><span>Google services — connect only what you want</span><div className="gw-services">{["gmail", "calendar"].map((svc) => { const health = selected.services?.[svc] || {}; const caps = (selected.catalog as any[]).filter((c) => c.service === svc); return <div key={svc} className="gw-svc"><div className="gw-svc-h"><i data-live={!!health.connected} /><strong>{health.label || svc}</strong><small>{health.connected ? "connected" : "not connected"}</small></div><div className="gw-caps">{caps.map((c) => { const granted = (selected.grantedBundles || []).includes(c.key); return <button key={c.key} className="gw-cap" data-granted={granted} disabled={granted || !selected.configured} title={c.why} onClick={() => connectGoogle(c.key)}>{granted ? `✓ ${c.label}` : `Connect · ${c.label}`}</button>; })}</div></div>; })}</div>{!selected.configured ? <p className="gw-note">Add Google OAuth credentials (client ID + secret) to enable connecting. Ask Jarvis to walk you through creating them.</p> : null}</div> : null}{selected.missing?.length ? <div className="as-section"><span>Missing setup</span><ul>{selected.missing.map((item: string) => <li key={item}>{item}</li>)}</ul></div> : null}<div className="as-actions"><button onClick={() => jarvis(`Diagnose the ${selected.label} connection. Explain its current status, exact blocker, safe setup steps, and how to verify it without exposing secrets.`)}>Diagnose with Jarvis</button></div></div> : <div className="as-empty">Select a provider.</div>}</section></div>
  </div>;
}
